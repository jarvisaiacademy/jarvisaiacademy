/**
 * Seed data for the `testimonials` collection — invented, and kept out of `src/` so it
 ...
 * not photographs of anyone. That is why this file lives beside the seed script rather than
 * in `src/`: the app no longer falls back to them, so an unseeded database shows no
 * testimonials at all instead of quoting a graduate who never enrolled.
 *
 * Replace this array with the academy's real alumni once they have agreed to be quoted —
 * their own names, roles, employers and photographs — then run `pnpm seed:content` against
 * the project. Nothing in `src/` needs to change when you do.
 */

export const TESTIMONIAL_POOL = [
  {
    name: "Gurpreet Kaur",
...
export function testimonialId(name) {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
